...
from backend.tts.tts_coqui import synthesize_coqui
...
def run_pipeline(in_audio: str, out_dir: str = "data/output", tts_choice: str = "espeak"):
    ...
    if tts_choice == "espeak":
        synthesize_espeak(hi_text, tts_wav)
    elif tts_choice == "coqui":
        synthesize_coqui(hi_text, tts_wav)
    else:
        raise ValueError(f"Unknown TTS choice: {tts_choice}")

    return {"en_text": en_text, "hi_text": hi_text, "tts_wav": tts_wav}

if __name__ == "__main__":
    # Test with Coqui TTS
    result = run_pipeline("data/tests/sample.wav", out_dir="data/output", tts_choice="coqui")
    print(result)
